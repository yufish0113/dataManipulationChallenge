...
*Question 2 Answer:
To query rows for shipments that are at least a certain number of days late using a variable, we can use the `@` symbol in `.query()` to reference Python variables. 
...
*Question 2 Answer: 
Duplicate `partID` in the right table will create multiple rows for the same shipment. This happens because each shipment record will match several category rows. It may cause data duplication and incorrect calculations.
...
*This visualization shows the on-time delivery rate across product categories. 
We can see clear differences in performance, with some categories achieving higher service levels than others. Categories with lower on-time rates may require operational review to identify potential fulfillment challenges. 
Overall, the chart helps management quickly compare service performance and prioritize improvements.*
